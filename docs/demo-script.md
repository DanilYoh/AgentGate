...
1. Show a clean `git status` and run `npx agentgate check` to get “No findings.”
...
3. Run `npx agentgate check` again. Point out the `secret-added` finding, its
   file and line, masked evidence, recommendation, and exit code 1.
4. Run `npx agentgate check --format sarif > agentgate.sarif` and show that the
   file is valid JSON and contains `[REDACTED]`, not the synthetic token.
